    ...
    try:
        with connection.cursor() as cursor:  # Usando o contexto para garantir que o cursor seja fechado
            cursor.execute("""
                ...
                VALUES (SEQ_USUARIO.NEXTVAL, :nome, :email, :password, :funcao)  -- A sequência foi alterada para SEQ_USUARIO
...
def inserir_produtor(email_usuario, cpf, cnpj, data_nascimento, cep, cidade, estado, pais, telefone, nome_fantasia, tipo_producao, tamanho_propriedade):
    try:
        with connection.cursor() as cursor:
            # Verificar se o CPF ou CNPJ já existem
            cursor.execute("SELECT 1 FROM produtor WHERE cpf = :cpf", cpf=cpf)
            if cursor.fetchone():
                print(f"Erro: CPF {cpf} já existe no banco de dados!")
                return

            cursor.execute("SELECT 1 FROM produtor WHERE cnpj = :cnpj", cnpj=cnpj)
            if cursor.fetchone():
                print(f"Erro: CNPJ {cnpj} já existe no banco de dados!")
                return
...
# Atualize a geração do CPF e CNPJ no loop para usar essas funções
